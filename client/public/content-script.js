...
window.addEventListener("message", async (event) => {
    ...
    } else if (event.data.type === "REQUEST_TOTAL_DAYS") {
        chrome.runtime.sendMessage({ type: "GET_TOTAL_DAYS" }, (response) => {
            console.log('response from background: ', response);
            const totalDays = response.totalDays;
            window.postMessage({ type: "RECEIVE_TOTAL_DAYS", totalDays: totalDays })
        })
    }
